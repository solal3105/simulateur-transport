...
export function useProgramme() {
  ...
  const lignesProgramme = [
    ...chantiers.map((c) => {
      ...
      return { id: c.id, nom: p.nom, cout: r.cout, voyageurs: r.voyageurs, annee: ouverture(c.mandat, r.duree), mandat: c.mandat, etale: c.etale, joueur: false }
    }),
    ...lignes.map((l) => ({
      id: l.id,
      nom: l.nom,
...
export function Programme() {
  ...
  return (
    <aside className="absolute top-24 bottom-0 left-0 z-10 hidden w-[340px] flex-col gap-6 overflow-y-auto border-r border-trait bg-white px-6.5 pt-6.5 pb-6 lg:flex">
          ...
          <ul>
            {courant.map((l) => (
              <li key={l.id}>
                ...
                >
                  <span className="grid size-8.5 shrink-0 place-items-center rounded-[10px] bg-rouge text-white">
      ...
      <p className="text-sm leading-relaxed text-gris">
        Choisissez des projets sur la carte ou tracez votre propre ligne. Quand vous avez fini, terminez le mandat : l’argent non dépensé
        ne passe pas au suivant.
      </p>
...
export function BarreBas() {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 z-20 grid grid-cols-2 gap-2 border-t border-trait bg-white px-4 pt-3 pb-6 lg:hidden">
      <Bouton genre="sable" iconeAGauche="pieces" taille="petit" className="min-h-13 justify-start" onClick={() => ouvrir({ type: 'leviers' })}>
        Trouver de l’argent
      </Bouton>
      <Bouton
        genre="rouge"
