...
`data/modele/frequentation.csv` rassemble 132 lignes de 23 villes, relevées le 23 septembre 2026 : métro, tram et bus à haut niveau de service. Chaque ligne donne sa valeur telle que publiée, son unité, son année et sa source, puis la valeur ramenée à un jour de semaine. Une valeur annuelle est divisée par 265, comme pour le calage lyonnais d'origine ; une valeur par jour de semaine est gardée telle quelle. Les sources sont les rapports d'activité des réseaux, leurs données ouvertes, l'observatoire Omnil pour l'Île-de-France et, pour quelques villes moyennes, la presse ; la colonne `remarque` le dit alors.

108 lignes sont retenues pour le calage ; l'une d'elles, la C4 de Mulhouse, est introuvable dans OpenStreetMap, et le calage porte donc sur 107 lignes de 22 villes. Les autres sont écartées pour une raison écrite dans la base : un tracé qui a changé depuis l'année du chiffre (lignes A et C de Bordeaux, T6 de Lyon, T3 de Marseille), une navette atypique (3 bis et 7 bis à Paris, T7 de Lyon), un comptage incertain (Valenciennes, gratuit jusqu'à 25 ans), une ligne qui en double d'autres sur presque tout son parcours (ligne 3 de Mulhouse), un métro à crémaillère (C à Lyon), une ligne trop récente pour avoir trouvé ses voyageurs, ou une liaison interurbaine.

Les unités ne sont pas partout les mêmes : voyages, validations, montées d'enquête. Deux cas sont traités à part. Le métro parisien compte ses entrées, sans les correspondances d'une ligne de métro à l'autre : il a son propre niveau dans la formule. Les bus sont de deux sortes, les bus en site propre (busway de Nantes, Mettis de Metz, BHNS de Marseille, Bordeaux, Strasbourg, Nîmes, Pau) et les lignes de bus renforcées sans site propre continu (Linéo de Toulouse, Chronostar de Rennes, Lianes de Lille, Chronobus de Nantes), notées dans la colonne `bus`.

Quand une ville ne publie qu'un chiffre pour deux lignes (le tram de Lille, R et T ; les deux trams du Havre ; le Mettis de Metz), les deux lignes sont réunies en une seule pour le calcul.
...
Le score est la moyenne des deux écarts, mesurés en logarithme. Chaque ligne pèse 1 sur la racine du nombre de lignes de sa ville, sinon les 26 lignes de Paris dicteraient la formule aux villes qui n'en ont que deux. Les écarts sans une ligne ou sans une ville se calculent par des formules exactes, vérifiées contre un recalage complet (écart de l'ordre de 10⁻¹² sur le logarithme) : les 23 980 formules sont jugées en moins d'une minute.

Deux règles viennent ensuite. Le moteur n'accepte que des formules où chaque variable agit dans le sens attendu pour un joueur : plus d'habitants ou de gares, plus de voyageurs ; plus de concurrence ou d'éloignement du centre, moins. Une formule qui ferait perdre des voyageurs à une ligne parce qu'elle a des correspondances colle un peu mieux aux chiffres, par un effet de double compte des bassins, mais elle apprendrait au joueur une règle fausse. Ensuite, beaucoup de formules font presque aussi bien que la meilleure et leur ordre change dès qu'on ajoute quelques lignes : le moteur réunit celles qu'on ne peut pas distinguer de la meilleure (l'écart ne dépasse pas son erreur type, mesurée ligne par ligne) et retient les variables présentes dans plus de la moitié d'entre elles.

## La formule retenue

Sur 107 lignes de 22 villes, la formule retenue est :

    voyageurs par jour = exp(constante de la ville + mode + 0,935 × bassin − 0,219 × log(1 + distance au centre en km) − 0,778 × concurrence)

Le bassin est le logarithme de 1 plus les habitants et 0,75 fois les emplois à moins de 1 000 m d'une station de métro, ou à moins de 400 m d'un arrêt des autres modes, plus 30 % de ceux qui vivent ou travaillent entre 400 m et 1 000 m. Le mode ajoute 0,343 pour un métro (41 % de voyageurs de plus qu'un tram au même bassin) et retire 0,740 pour un bus en site propre (52 % de moins) ; une ligne de bus renforcée sans site propre perd encore 0,451. La constante de chaque ville réunit une constante commune, la taille de son réseau (0,311 fois le logarithme du nombre de lignes de métro et de tram) et le niveau propre de la ville.

Parmi les 31 formules indiscernables de la meilleure, la concurrence est présente dans toutes, la distance au centre dans 87 %, la taille du réseau dans 65 %. Les correspondances et les gares ne sont dans aucune ; le bassin de vie et le bassin d'emploi de la ville dans une sur cinq, leur effet étant déjà porté par le niveau propre de chaque ville.

Sur une ligne absente du calage, l'estimation s'écarte du réel de 29 % en moyenne quand la ville est connue, et de 36 % quand elle ne l'est pas. Le réel se situe entre 0,62 et 1,39 fois l'estimation pour huit lignes sur dix : c'est la fourchette affichée dans le jeu.

Le téléphérique n'a aucune ligne dans le calage. Son écart au tram est estimé sur les deux téléphériques dont on connaît la fréquentation, Téléo à Toulouse et celui de Brest, prédits comme des trams : −0,37, soit 31 % de voyageurs de moins qu'un tram. C'est fragile.
...
Le jeu ne calcule pas avec la grille nationale mais avec les carreaux de chaque ville (`public/data`), faits à des dates et parfois par des chaînes différentes : à Lyon, ils comptent un peu plus de monde près des arrêts de tram. `scripts/modele/recaler-jeu.ts` fait estimer par le code du jeu les lignes réelles de chaque ville et ajoute à la constante de la ville l'écart médian avec le moteur : −0,107 à Lyon, +0,064 à Toulouse, +0,017 à Marseille, +0,053 à Nice, −0,038 à Paris. Avant ce recalage, le jeu retrouvait les chiffres du moteur à 10 % près en moyenne ; les plus gros écarts restants viennent de lignes qui sortent du territoire du jeu, comme les trams T5 et T6 à Paris.

À Paris, le métro compte ses voyageurs aux entrées, sans les correspondances : le jeu estime les lignes de métro du joueur de la même façon, pour qu'elles se comparent aux lignes existantes (ajustement de −0,732).

## Ce que la formule ne voit pas

La formule ne connaît ni la vitesse ni la fréquence des lignes, ni les grands équipements comme les hôpitaux, les campus ou les stades. Elle se trompe le plus sur des lignes à part : le T5 de Lyon vers Eurexpo, qu'elle surestime, ou le busway de Nantes, bien meilleur que la moyenne des bus en site propre, qu'elle sous-estime. Les unités de fréquentation diffèrent d'un réseau à l'autre ; le niveau propre de chaque ville absorbe l'essentiel de ces écarts, pas les différences entre modes d'une même ville.
